...
export const Title = {
  PageTitle: ({ titulo, className }: TitleProps) => {
    return <h1 className={`font-OpenSans text-2xl font-bold text-typography-first capitalize ${className || ''}`}>{titulo}</h1>;
  },
  Subtitle: ({ titulo, className }: TitleProps) => {
    return <h2 className={`font-OpenSans text-base font-normal text-typography-second capitalize ${className || ''}`}>{titulo}</h2>;
  },
  DrawerTitle: ({ titulo, className }: TitleProps) => {
    return <h3 className={`font-OpenSans text-xl font-semibold text-typography-first capitalize ${className || ''}`}>{titulo}</h3>;
  },
  TableHeader: ({ titulo, className }: TitleProps) => {
    return <h4 className={`font-OpenSans text-base font-bold text-typography-first capitalize ${className || ''}`}>{titulo}</h4>;
  },
  InputTitle: ({ titulo, className }: TitleProps) => {
    return <h5 className={`font-OpenSans text-sm font-semibold text-typography-first capitalize ${className || ''}`}>{titulo}</h5>;
  },
};